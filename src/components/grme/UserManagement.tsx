...
export default function UserManagement({ onClose, dataEntryWindow, adminEvents, adminName, onRefreshData, domains, dzongkhags, thromdes }: UserManagementProps) {
  ...
  const [newDzongkhag, setNewDzongkhag] = useState("");
  ...
  useEffect(() => {
    const defaultDzongkhagId = dzongkhags[0]?.id || "";
    if (defaultDzongkhagId && !newDzongkhag) setNewDzongkhag(defaultDzongkhagId);
    if (defaultDzongkhagId && !editDzongkhag) setEditDzongkhag(defaultDzongkhagId);
  }, [dzongkhags, editDzongkhag, newDzongkhag]);
  ...
  const handleAdd = async () => {
    ...
    setNewDzongkhag(dzongkhags[0]?.id || "");
    setNewThromde("");
